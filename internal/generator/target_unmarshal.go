...
func (tg *Target) genMapFieldUnmarshal(f *protogen.Field) (c []Code) {
	...
	loop = append(loop,
		...
		Err().Op("=").Add(tg.idents.unmarshal).Call(Id("v"), Op("&").Id("mv"), Id("o").Op("...")),
		If(Err().Op("!=").Nil()).Block(
...
func (tg *Target) genMessageFieldUnmarshal(f *protogen.Field) []Code {
	return []Code{
		...
		If(Id("m").Index(Lit(tg.attrName(f))).Op("!=").Nil()).Block(
			...
			Err().Op("=").Add(tg.idents.unmarshal).Call(Id("m").Index(Lit(tg.attrName(f))), Id("x").Dot(f.GoName), Id("o").Op("...")),
			If(Err().Op("!=").Nil()).Block(
...
func (tg *Target) genOneOfFieldUnmarshal(f *protogen.Field) []Code {
	...
	case f.Message != nil:
		...
		marshal = append(marshal,
			...
			Err().Op("=").Add(tg.idents.unmarshal).Call(Id("m").Index(Lit(tg.attrName(f))), Id("mo").Dot(f.GoName), Id("o").Op("...")),
		)
	default:
...
func (tg *Target) genMessageUnmarshal(f *File, m *protogen.Message) error {
	...
	f.Func().
		...
		Params(Id("m").Map(String()).Qual(dynamodbtypes, "AttributeValue"), Id("o").Op("...").Add(tg.idents.decopt)).
